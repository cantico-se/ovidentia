...
class bab_dumpToDb
{
    ...
    function getDatabase()
    {
        ...
        if ($oResult === true) {
            if (! empty($install->clearDb)) {
                ...
                if (mb_strtolower($aDbCharset['Value']) !== $sDbCharsetToCompare) {
                    $this->error->add($this->trans->str('Incompatible database charset, you have selected : ') . $sDBCharset . $this->trans->str(' and the charset of the database is : ') . $aDbCharset['Value']);
                    return false;
                }
            }
        }
        ...
        if (! $oResult || (isset($createdatabase) && $createdatabase === true)) {
            $sQuery = '';
            if ('utf8' === $sDBCharset) {
                $sQuery = 'CREATE DATABASE `' . $install->babDBName . '` ' . 'CHARACTER SET utf8 ' . 'DEFAULT CHARACTER SET utf8 ' . 'COLLATE utf8_general_ci ' . 'DEFAULT COLLATE utf8_general_ci ';
            } else {
                $sQuery = 'CREATE DATABASE `' . $install->babDBName . '` ' . 'CHARACTER SET latin1 ' . 'DEFAULT CHARACTER SET latin1 ' . 'COLLATE latin1_swedish_ci ' . 'DEFAULT COLLATE latin1_swedish_ci ';
            }
...
function _createFmDirectories()
{
    ...
    if (! is_dir($sFmUploadPath) && ! @mkdir($sFmUploadPath, 0777)) {
...
class installInfos
{
    ...
    public function testVars()
    {
        ...
        if (! empty($this->babUploadPath)) {
            ...
            if (! is_dir($this->babUploadPath . '/tmp/') && ! @mkdir($this->babUploadPath . '/tmp/')) {
                $error->add($trans->str('Can\'t create upload tmp directory'));
                return false;
            }
...
<body>
    ...
    <div id="form">
        ...
        ?>
        <p><?php echo $trans->str('Congratulations, ovidentia is now configured. You can log in with the default account') ?></p>
        <ul>
            <li><?php echo $trans->str('Login ID') ?> : <strong>admin@admin.bab</strong></li>
            <li><?php echo $trans->str('Password') ?> : <strong>012345678</strong></li>
        </ul>
        <p>
            <a href="index.php?tg=login"
                onclick="window.open('http://www.ovidentia.org/premiers-pas', '', 'width=500,height=400,scrollbars=yes');"><?php echo $trans->str('Go to login page') ?></a>
        </p>
        <?php
        } else {
        ?>
        ...
        <form method="post"
            ...
            <dl>
                <fieldset>
                    ...
                    <dt>
                        <label for="babDBHost"><?php echo $trans->str('Database host') ?> :</label><input
                            type="text" id="babDBHost" name="babDBHost"
                            value="<?php echo $install->babDBHost; ?>" />
                    </dt>
                    <dt>
                        <label for="babDBName"><?php echo $trans->str('Database name') ?> :</label><input
                            type="text" id="babDBName" name="babDBName"
                            value="<?php echo $install->babDBName; ?>" />
                    </dt>
                    ...
                    <dt>
                        ...
                        <?php
                        if ('utf8' == $install->babDBCharset) {
                            echo '<option value="utf8" selected>utf8</option><option value="latin1">latin1</option>';
                        } else {
                            echo '<option value="utf8">utf8</option><option value="latin1" selected>latin1</option>';
                        }
                        ?>
                        </select>
                    </dt>
                    ...
                    <dt>
                        <label for="babDBName"><?php echo $trans->str('Drop database') ?> :</label><input
                            type="checkbox" id="clearDb" name="clearDb"
                            <?php if ($install->clearDb) echo 'checked="checked"'; ?> />
                    </dt>
                    <dd><?php echo $trans->str('If the database exists, it will be dropped and data will be lost') ?></dd>
                    <dt>
                        <label for="babDBLogin"><?php echo $trans->str('Login ID') ?> :</label><input
                            type="text" id="babDBLogin" name="babDBLogin"
                            value="<?php echo $install->babDBLogin; ?>" />
                    </dt>
                    <dt>
                        <label for="babDBPasswd"><?php echo $trans->str('Password') ?> :</label><input
                            type="password" id="babDBPasswd" name="babDBPasswd" />
                    </dt>
                </fieldset>
                ...
                <fieldset>
                    ...
                    <dt>
                        <label for="babInstallPath"><?php echo $trans->str('Relative path to ovidentia core') ?> :</label><input
                            type="text" id="babInstallPath" name="babInstallPath"
                            value="<?php echo $install->babInstallPath;?>" />
                    </dt>
                    <dt>
                        <label for="babUploadPath"><?php echo $trans->str('Upload directory') ?> :</label><input
                            type="text" id="babUploadPath" name="babUploadPath"
                            value="<?php echo $install->babUploadPath;?>" />
                    </dt>
                    <dd>
                    <?php
