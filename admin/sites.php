...
function siteModify($id)
	...
	class temp
		...
		function temp($id)
			...
			$this->namecomposeorder_title = bab_translate("User name composition");
			$this->changenickenable_title = bab_translate("User can modifiy his nickname");
			$this->changepassenable_title = bab_translate("User can modifiy his password");
			$this->cookieident_title = bab_translate("Automatic connection");
			...
			$this->maxgroupsize_title = bab_translate("File manager max group directory size");
			$this->maxusersize_title = bab_translate("File manager max user directory size");
			$this->maxtotalsize_title = bab_translate("File manager max total size");
...
function siteUpdate($id, $name, $description, $lang, $style, $siteemail, $skin, $register, $confirm, $mailfunc, $server, $serverport, $imgsize, $group, $smtpuser, $smtppass, $smtppass2, $langfilter,$maxtotalsize, $maxusersize, $maxgroupsize, $maxfilesize, $uploadpath, $babslogan, $cookieident, $changepassenable, $changenickenable, $namecomposeorder)
	{
	global $babBody;
	if( empty($name))
		{
	...
	if( !str_is_int($maxtotalsize) || !str_is_int($maxusersize) || !str_is_int($maxgroupsize) || !str_is_int($maxfilesize))
		{
	...
	if( $db->db_num_rows($res) > 0)
		...
		if( !is_numeric($imgsize))
			$imgsize = 25;
		$req .= "description='".$description."', lang='".$lang."', adminemail='".$siteemail."', skin='".$skin."', style='".$style."', registration='".$register."', email_confirm='".$confirm."', mailfunc='".$mailfunc."', smtpserver='".$server."', smtpport='".$serverport."', imgsize='".$imgsize."', idgroup='".$group."', smtpuser='".$smtpuser."', smtppassword=ENCODE(\"".$smtppass."\",\"".$GLOBALS['BAB_HASH_VAR']."\"), langfilter='" .$langfilter. "',maxtotalsize='".$maxtotalsize."', maxusersize='".$maxusersize."', maxgroupsize='".$maxgroupsize."', maxfilesize='".$maxfilesize."', uploadpath='".$uploadpath."', babslogan='".$babslogan."', cookieident='".$cookieident."', changepassenable='".$changepassenable."', changenickenable='".$changenickenable."', namecomposeorder='".$namecomposeorder."' where id='".$id."'";
		$db->db_query($req);
		}
...
if( isset($modify))
	...
	if( !empty($Submit))
		{
		if(!siteUpdate($item, $name, $description, $lang, $style, $siteemail, $skin, $register, $confirm, $mailfunc, $server, $serverport, $imgsize, $group, $smtpuser, $smtppass, $smtppass2, $babLangFilter->convertFilterToInt($langfilter),$maxtotalsize, $maxusersize, $maxgroupsize, $maxfilesize, $uploadpath, $babslogan, $cookieident, $changepassenable, $changenickenable, $namecomposeorder))
			$idx = "modify";
		}
