...
function babTopicsSection($cat, $close)
	...
	while( $row = $babDB->db_fetch_array($res))
		{
		if(in_array($row['id'], $babBody->topview) || $GLOBALS['BAB_SESS_USERID'] == $row['id_approver'])
		{
			if( $close )
			{
				$this->count = 1;
	...
	if( $i < $this->count)
		...
		if( $res && $babDB->db_num_rows($res) > 0)
			...
			if( $BAB_SESS_USERID == $this->arr['id_approver'])
				...
				$req = "select id from ".BAB_COMMENTS_TBL." where id_topic='".$this->arr['id']."' and confirmed='N'";
				$res = $babDB->db_query($req);
				if($babDB->db_num_rows($res) > 0)
					$this->newc = "c";
				}
