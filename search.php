...
// ---------------------------------------- SEARCH ARTICLES AND ARTICLES COMMENTS ---------
			...
			if( empty($item) || $item == "e")
                ...
                if ($idfile != "") 
					{
					$req = "insert into filresults select F.id, F.name title, F.id_owner, description, DATE_FORMAT(created, '%d-%m-%Y') datec, DATE_FORMAT(modified, '%d-%m-%Y') datem, path, bgroup, concat( U.lastname, ' ', U.firstname ) author, folder from ".BAB_FILES_TBL." F, ".BAB_USERS_TBL." U, ".BAB_FM_FOLDERS_TBL." R where F.author=U.id and F.id_owner=R.id and (".finder($this->like,"F.name",$option,$this->like2)." or ".finder($this->like,"description",$option,$this->like2)." or ".finder($this->like,"keywords",$option,$this->like2)." or ".finder($this->like,"F.path",$option,$this->like2)." or ".finder($this->like,"R.folder",$option,$this->like2).") and F.id_owner in (".substr($idfile,0,-1).") ". $grpfiles ." and state='' and confirmed='Y' order by ".$order;
                    $this->db->db_query($req);
                    }
