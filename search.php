	...
	class tempb
			...
			foreach($this->htmlfields as $key=>$val)
				...
				if (is_array($val)) {
					foreach($val as $k2 => $v2) {
						$this->htmlfields[$key][$k2] = bab_toHtml($v2);
					}
				} else {
		...
		function getnextdir() 
			...
			if( $l < $this->countdirs)
				...
				$this->topicid = $arr['id'];
				$this->selected = isset($this->fields['directoryid']) && $this->fields['directoryid'] == $arr['id'];
		...
		function selectname() 
			...
			if( $i < $this->countfields)
				...
				if ( isset($this->fields['f'][$this->j]) && $this->fields['f'][$this->j] == $arr['name'])
		...
		function getnextfieldtosearch() 
			...
			if( $j < FIELDS_TO_SEARCH)
				...
				$this->value = isset($this->htmlfields['v'][$j]) ? $this->htmlfields['v'][$j] : '';
				$this->j = $j;
				$j++;
				return true;
				}
...
// ---------------------------------------- SEARCH ARTICLES AND ARTICLES COMMENTS ---------
			...
			if( count($arrview) && (empty($item) || $item == 'directories'))
				{
				$id_directory = isset($this->fields['directoryid']) ? $this->fields['directoryid'] : '';
				...
				for($i = 0 ; $i < FIELDS_TO_SEARCH ; $i++)
					{
					$dirselect = isset($this->fields['f'][$i]) ? $this->fields['f'][$i] : '';
					$dirfield = isset($this->fields['v'][$i]) ? $this->fields['v'][$i] : '';
					if ($dirfield !="") 
						{
