...
function Dialog(url, action, init,param) {
	...
	if (document.all) {	// here we hope that Mozilla will never support document.all
		...
		return Dialog._geckoOpenModal(url, action, init,param);
	}
};
...
Dialog._geckoOpenModal = function(url, action, init,param) {
	if (typeof param == "undefined")
	{param = 'toolbar=no,menubar=no,personalbar=no,width=10,height=10,scrollbars=no,resizable=no';}
	var dlg = window.open(url, "ha_dialog",param);
	Dialog._modal = dlg;
