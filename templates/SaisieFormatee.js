	...
	var BUTTON_IMAGE_PATH = "templates/Images";
...
//-----------------------------------------------------------------
	...
	function EditorOnCreateHyperlink(id)
	{
		...
		var link = prompt("Saisissez le lien (exemple : http://www.ovidentia.org) :", anchor ? anchor.href : "http://");
		if (link && link != "http://") {
	...
	function EditorValidateMode(id)
	{
		...
		alert("Veuillez décocher la case \"Afficher le code HTML\" pour utiliser la barre d'outils.");
		eval(EDITOR_COMPOSITION_PREFIX + id).focus();
		return false;
	}
...
function document.body.onload() {
  //editor.SetHTML(document.all.plainmsg.innerHTML);
  	for ( i=0 ; i<NumZone ; i++ )
	{
		...
		//editor.
		editor.SetHTML(document.forms[ZForm[i]].elements[Zone[i]].value);
  	}
...
function SetVals() {
	...
	{
		EDITOR_COMPOSITION_PREFIX = Zone[i];
		document.forms[ZForm[i]].elements[Zone[i]].value = editor.GetHTML();
	}
