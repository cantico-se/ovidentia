...
class BAB_TM_TaskTime
{
    ...
    function getTaskPredecessor($aTask)
    {
        ...
        //Si la tache a un predEcesseur et qu'elle est liee avec celui-ci
        if(0 !== $iIdPredecessor)
        {
        ...
        while($oLoopStartDate->getTimeStamp() < $oLoopEndDate->getTimeStamp())
         {
            if($oLoopStartDate->getYear() > $iYear)
             ...
             if(!array_key_exists($oLoopStartDate->getIsoDate(), $aNWD))
             {
                ...
                {
                    $iDayToAdd = 0;
                    //Si la start periode est superieure ou egale a la end periode
...
$oEndDate = $oLoopDate->cloneDate();
        ...
        //Tant que l'on a pas atteint la duree
...
//*/
    ...
    function computeRemainingDates(&$oRemainStartDate, &$oRemainEndDate)
    {
        ...
        {
            //echo ("La date de debut est superieure a la date de fin");
            $oRemainStartDate = $this->cloneStartDate();
        }
        else
        {
            //echo ("La date de debut est inferieur a la date de fin");
        ...
        //Du debut de la tache jusqu'a fin du taux de completion. (barre noir).
        //Ce calcul ne tient pas compte des jours non travailles
        ...
        //Fin de completion jusqu'a la fin de la tache. (barre blanche).
        //Ce calcul ne tient pas compte des jours non travailles
        ...
        //Entre la fin de completion et ce qui reste a faire, prise du nombres de secondes
        //ce calcul tient compte des jours non travailles car a partir de la date du jour
        //il va falloir rajouter ce qui reste a faire et ceci en tennat compte des jours non travailles
        $iWorkSec1 = BAB_TM_TaskTime::getWorkingSecondsBetween($oToDoStart, $oToDoEnd);
        $fRemainDuration = ($iWorkSec1 / $this->m_iNbSeconds);

        //A partir de la date du jour, calcul de ce qui reste a faire en tenant compte des jours non travailles (barre rose)
...
class BAB_TM_TaskTimeDate extends BAB_TM_TaskTime
{
    ...
    function init($aTask)
    {
        ...
        if('0000-00-00 00:00:00' !== $sIsoStartDate)
        {
            ...
            {
                //A ce stade la date de debut reel est renseignee mais pas la date de fin
                //il faut donc calculer en incluant les non workings days ainsi que les
                //plages de travail la date de fin theorique

                //Recupere le temp en seconde entre deux dates, tient compte des jours travailles
                ...
                //Recalcule la date de fin theorique en fonction des jours travailles
                ...
                /*
                //Si le calcul est bon normalement les deux temps (bab_debug en commentaire) doivent etre egaux
...
class BAB_TM_TaskTimeDuration extends BAB_TM_TaskTime
{
    ...
    function init($aTask)
    {
        ...
        //Calcul la duree effective en fonction des jours non travailles
...
//*/
        ...
        if(0 !== $iIdPredecessor && BAB_TM_END_TO_START === $this->m_iLinkType)
        {
            ...
            // Appele une fois de trop pas sur
            $oGanttTask->computeRemainingDates($oRemainStartDate, $oRemainEndDate);
            $this->m_oPlannedStartDate	= $oRemainEndDate->cloneDate();
