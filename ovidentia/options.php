    ...
    switch($authentification)
        ...
        case BAB_AUTHENTIFICATION_AD:
            if( !empty($babBody->babsite['ldap_encryptiontype']))
                ...
                if( $ret === false )
                    {
                    $babBody->msgerror = bab_translate("Connection failed. Please contact your administrator");
                    return false;
                    }
                ...
                if( !$ret )
                    {
                    $ldap->close();
                    $babBody->msgerror = bab_translate("Binding failed. Please contact your administrator");
                    return false;
                    }
                else
                    {
                    ...
                    if( $entries === false )
                        {
                        $ldap->close();
                        $babBody->msgerror = bab_translate("Authentification failed. Please verify your login ID and your password");
                        return false;
                        }
            ...
            if( !empty($babBody->babsite['ldap_encryptiontype']))
                ...
                if( !$ret )
                    ...
                    if( $entries === false || $entries['count'] == 0 )
                        {
                        $babBody->msgerror = bab_translate("User not found in the directory");
                        $ldap->close();
                        return false;
                        }
                    }
