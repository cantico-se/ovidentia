...
class bab_InstallRepository
{
     ...
     *
     * @param string $name
     *            Name of addon (filename without version and extension)
...
class bab_InstallRepositoryFile
{
    ...
    public function install($updateProgess = false)
    {
        ...
        if (! $ini->isValid()) {
            bab_installWindow::message(bab_toHtml(sprintf(bab_translate('Package %s is not valid, please check dependencies'), $install->getArchive())));
        ...
        if ($install->install($ini)) {
            if (! unlink($install->getArchive())) {
                bab_installWindow::message(bab_toHtml(sprintf(bab_translate('Failed to delete the temporary package %s'), $install->getArchive())));
            }
        }
     ...
     * Download the file
     *
     * @param resource $rfp                     File pointer resource, readable source file
     * @param resource $wfp                     File pointer resource, writable destination
...
class bab_InstallSource
{
    ...
    private function installCore(bab_CoreIniFile $ini)
    {
        ...
        bab_installWindow::message(
            sprintf(
                '<script type="text/javascript">document.location.href = \'%s\'</script> <a href="%s">Install</a>',
                $upgrade_page,
                bab_toHtml($upgrade_page)
            )
        );
...
class bab_installWindow
{
     ...
     *            array or string the function must return a boolean
     */
