...
class bab_functionalities {
	...
	private function recordLink($path, $funcpath, $include_file, $linkfilename) {
		...
		$content = '<?php if (file_exists(\''.$this->getIncludeFilePath($include_file).'\') && (class_exists(\''.$classname.'\',false) || false !== include_once \''.$this->getIncludeFilePath($include_file).'\')) { return \''.$classname.'\'; } else { return false; } ?>';
