...
function bab_updateTopicsCategory($id_category, $name, $description, $benabled, $template, $disptmpl, $topcatid, $dgowner=null)
{
    ...
    else
    {
        ...
        if ($old['id_parent'] !== (string) $topcatid) {
            ...
            $babDB->db_query("UPDATE ".BAB_TOPCAT_ORDER_TBL." SET id_parent=".$babDB->quote($topcatid).", ordering=".$babDB->quote($ord)." WHERE id_topcat=".$babDB->quote($id_category)." and type='1'");
        }
    }
...
function bab_updateTopic($id_topic, $name, $description, $idCategory, &$error, $topicArr = array())
{
    ...
    if ($topicArr['id_cat'] !== $old['id_cat'])
    {
        ...
        $babDB->db_query("UPDATE ".BAB_TOPCAT_ORDER_TBL." SET ordering=".$babDB->quote($ord).", id_parent=".$babDB->quote($idCategory)." WHERE id_topcat=".$babDB->quote($id_topic) ." AND type='2'");
