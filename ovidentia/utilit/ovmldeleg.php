    ...
    public function setOvmlContext(babOvTemplate $ctx)
    {
        ...
        if( $userid != '')
            ...
            if( $delegationid === false || $delegationid === '' )
                ...
                {
                $delegationid = explode(',', $delegationid);
                $this->res = $babDB->db_query("select dgt.* from ".BAB_DG_ADMIN_TBL." dat left join ".BAB_DG_GROUPS_TBL." dgt on dat.id_dg=dgt.id where dat.id_user='".$babDB->db_escape_string($userid)."' and id IN (".$babDB->quote($delegationid).") order by dgt.name asc");
                }
            $this->count = $babDB->db_num_rows($this->res);
            }
        else
