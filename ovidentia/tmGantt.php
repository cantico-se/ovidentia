...
//*/
    ...
    function selectTask($sStartDate, $iStartWeekDay)
    {
        ...
        if(false != $this->m_result)
        {
            ...
            while(false != $this->m_result && false != ($datas = $babDB->db_fetch_assoc($this->m_result)))
            {
                ...
                //recupere tous les descendant afin de savoir si ils sont dans la zone
                //d'affichage
    ...
    function initDates($sStartDate, $iStartWeekDay)
    {
        ...
        $this->m_sPrevMonthUrl = bab_toHtml($sUrlBase . urlencode(date("Y-m-d", $oDate->getTimeStamp())));

        $oDate = BAB_DateTime::fromTimeStamp($this->m_aDisplayedStartDate[0]);
        $oDate->add(-7, BAB_DATETIME_DAY);
        $this->m_sPrevWeekUrl = bab_toHtml($sUrlBase . urlencode(date("Y-m-d", $oDate->getTimeStamp())));

        $oDate = BAB_DateTime::fromTimeStamp($this->m_aDisplayedStartDate[0]);
        $oDate->add(7, BAB_DATETIME_DAY);
        $this->m_sNextWeekUrl = bab_toHtml($sUrlBase . urlencode(date("Y-m-d", $oDate->getTimeStamp())));

        $oDate = BAB_DateTime::fromTimeStamp($this->m_aDisplayedStartDate[0]);
        $oDate->add(1, BAB_DATETIME_MONTH);
        $this->m_sNextMonthUrl = bab_toHtml($sUrlBase . urlencode(date("Y-m-d", $oDate->getTimeStamp())));
    ...
    function setDates($sStartDate, $iStartWeekDay)
    {
        ...
        // To start on a specific day of the week.
...
class BAB_TM_GanttTaskBase
{
    ...
    function createPeriod($iTaskStartDateTs, $iTaskEndDateTs)
    {
        ...
        //Tous les carres ont des bordures des quatres cotes de 1 px
        $iBorderTop		= 1;
        $iBorderLeft	= 1;
        $iBorderRight	= 1;
        $iBorderBottom	= 1;
...
if($oRemainEndDate->getTimeStamp() > $this->m_iDisplayedStartDateTs && $oRemainStartDate->getTimeStamp() < $this->m_iDisplayedEndDateTs)
            ...
            //Raccorde la date de fin de la tache a la date de debut de ce qui reste a faire
